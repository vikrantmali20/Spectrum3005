...
Imports System.IO
Public Class GiftVoucherReport
    ...
    Public Function GenerateDayCloseReport(request As SpectrumCommon.DayCloseReportModel, reportPath As String) As Boolean Implements IReports.GenerateDayCloseReport
        Try
            ...
            Dim ds As DataSet = GetGVReportDataSet(request)
            If ds.Tables IsNot Nothing AndAlso ds.Tables.Count > 0 AndAlso Not String.IsNullOrEmpty(path) Then
                ...
                Dim sheet1 As ISheet = hssfworkbook.CreateSheet("Gift voucher report")
                Dim siteName As String = GetSiteName(request.SiteCode)
                Dim lastRow As IRow = sheet1.CreateRow(0)
                lastRow.CreateCell(0).SetCellValue("Name : Gift voucher report")
                ...
                lastRow.CreateCell(0).SetCellValue("Gift voucher report for period : " & request.FromDate.ToShortDateString() & " to " & request.ToDate.ToShortDateString())
                SetCellStyle(lastRow, GetBoldFont(hssfworkbook))
    ...
    Private Function GetGVReportDataSet(ByVal request As DayCloseReportModel) As DataSet
        Try
            Dim query As String = "exec USP_GiftVoucherReport '" & request.FromDate.ToString("yyyy-MM-dd") & "' ,'" & request.ToDate.ToString("yyyy-MM-dd") & "' ,'" & request.ExpiryDate.ToString("yyyy-MM-dd") & "',1,'''" & request.SiteCode & "'''"
    ...
    Private Function GetDirectoryPath(ByRef request As DayCloseReportModel) As String
        Try
            ...
            End If
            path = DayCloseReportPath & "\GiftVoucherReport_" & siteName & "_" & request.FromDate.ToString("dd-MM-yyyy") & "_To_" & request.ToDate.ToString("dd-MM-yyyy") & ".xls"
            Return path
        Catch ex As Exception
            LogException(ex)
            Return Nothing
