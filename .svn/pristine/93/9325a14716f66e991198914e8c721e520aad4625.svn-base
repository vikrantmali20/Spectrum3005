...
Public Class frmonlineOrders
    ...
    Private Sub GridColumnSettings()
        Try
            ...
            End If
            '##
            Dim displayColumns As String = CouponColumn & "InvoiceNo,OrderNo,CustomerName,DeliveryPerson,SalesPerson" & EcomerceColumns & ",BillAmount,BalanceAmount,Address,BillCreatedTime,ElapsedTime,TotalAmt,FloatAmtReturned,Dispatch,DispatchTime,DeliveryPartner,TenderTypeCode"
            ...
            grdCreditSales.Cols("OrderNo").Caption = "Order No"
            grdCreditSales.Cols("OrderNo").Visible = True
            grdCreditSales.Cols("OrderNo").AllowEditing = False
    ...
    Private Sub frmNCreditSales_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            lblChannelPartner.Visible = False
            cmbChannelPartner.Visible = False
            Call BindCreditSales()
            Dim dChannelPartner As DataTable = FillChannelPartnerComboBox()
            CreateDynamicRadioBtn(dChannelPartner)
    ...
    Private Sub BindCreditSales()
        Try
            ...
            Dim refreshcheck As String = ""
            refreshcheck = SelectedDeliveryPartnerRadioBtn()
            dtCreditSales = objCreditSales.GetOnlineOrders(clsAdmin.SiteCode)
            If dtCreditSales IsNot Nothing Then
                ...
                'ElseIf dtCreditSales.Select("orderstatus='" & selectedOrderStatusText & "'").Length = 0 Then
                '    dt = dtCreditSales.Copy()
                '    dt.Clear()
                'Else

                '    dt = dtCreditSales
                'End If
                If Not String.IsNullOrEmpty(refreshcheck) AndAlso refreshcheck.ToString.ToUpper.Equals("UBEREATS") Then
                    selectedOrderStatusText = "Accepted"
                    If dtCreditSales.Select("orderstatus='" & selectedOrderStatusText & "' AND deliverypartner ='" & refreshcheck & "'").Length <> 0 Then
                        dt = dtCreditSales.Select("orderstatus='" & selectedOrderStatusText & "' AND deliverypartner ='" & refreshcheck & "'").CopyToDataTable
                    ElseIf dtCreditSales.Select("orderstatus='" & selectedOrderStatusText & "' AND deliverypartner ='" & refreshcheck & "'").Length = 0 Then
                        dt = dtCreditSales.Copy()
                        dt.Clear()
                    Else
        ...
        Try
            ...
            If grdCreditSales.Rows.Count > 1 Then
                ...
                ElseIf (grdCreditSales.Rows(grdCreditSales.Row)("DocType").ToString().ToLower() = "Cash memo".ToLower()) Then
                    ...
                    Dim objCMPrint As New clsCashMemoPrint(CashMemoNo, False, False, clsDefaultConfiguration.SalesPersonApplicable, clsDefaultConfiguration.TaxDetailsRequired, clsDefaultConfiguration.PrintPreivewReq, "L40", dtPrinterInfo, clsDefaultConfiguration.IsDisplayTotalSaving) '0000413
    ...
    Private Sub cmdDispatch_Click(sender As Object, e As EventArgs) Handles cmdDispatch.Click
        Try
            ...
            Dim CMD2 As String = "Select OrderStatus from DPACashMemoHdr where billno ='" + grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo") + "'"
            dt1 = objCM.GetFilledTable(CMD2)
            Dim status1 As String = dt1.Rows(0)("OrderStatus").ToString()

            If status1.ToString.ToUpper.Equals("VOID") Or status1.ToString.ToUpper.Equals("VOIDNOTIFY") Then
                ShowMessage("Already order is rejected", getValueByKey("CLAE05"))
                selectedOrderStatusText = cmbOrderStatus.Text
                cmdRefreshGrid_Click(Nothing, EventArgs.Empty)
                Exit Sub
            End If



            If grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo").ToString().Substring(0, 2) = "UB" Then
                Dim UberOrderToken As String = ""
                Dim AcceptOrderOutput As String = ""
                If GenerateUberAcceptAPI(grdCreditSales.Rows(grdCreditSales.Row)("Orderno").ToString()) Then
                    Dim CMD1 As String = "UPDATE DPACashMemoHdr set OrderStatus ='Accept' where billno= '" + grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo") + "'"
                    objCM.InsertOrUpdateRecord(CMD1)
                    OrderStatusUpdates("ORDER_APPROVED_AT_STORE", False, True)
                Else
                    writeLogForOnlineOrder("accept Api failed")
                End If

            End If

            Dim dt As New DataTable
            Dim CMD As String = "Select OrderStatus from DPACashMemoHdr where billno ='" + grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo") + "'"
            dt = objCM.GetFilledTable(CMD)
            Dim status As String = dt.Rows(0)("OrderStatus").ToString()
            If status = "Accept" Then
                CashMemoNo = objCM.getDocumentNo("CM", clsAdmin.SiteCode)
                CashMemoNo = GenDocNo("C" & clsAdmin.TerminalID.Substring(clsAdmin.TerminalID.Trim.Length - 2, 2) & clsAdmin.SiteCode.Substring(clsAdmin.SiteCode.Trim.Length - 3, 3) & clsAdmin.Financialyear.Substring(clsAdmin.Financialyear.Length - 2, 2), 15, CashMemoNo)
                objcomm.MooveOnlineOrdertoMainTables(clsAdmin.SiteCode, grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo").ToString(), CashMemoNo)
                'objcomm.MooveOnlineOrdertoMainTables(clsAdmin.SiteCode, docno)
                PrintKOT(False)
                CmdPrint_Click(Nothing, EventArgs.Empty)
                ...
                '  cmdRefreshGrid_Click(Nothing, EventArgs.Empty)
            Else
                ShowMessage("Order is Not Accepted yet... Please Accept Order first", getValueByKey("CLAE05"))
                Exit Sub
            End If
            Dim obj As New frmNHomeDelivery
            obj.IsDispatch = True
            If grdCreditSales.Row > 0 Then
                Dim Billno As String = grdCreditSales.Rows(grdCreditSales.Row)("BillNo")
                Dim dtHD As New DataTable
                dtHD = objCreditSales.GetHomeDeliveryDataZomato(Billno, clsAdmin.SiteCode)
                If Not dtHD Is Nothing And dtHD.Rows.Count > 0 Then
                    ...
                    Else
                        ...
                        OrderStatusUpdates("ORDER_DISPATCHED_FROM_STORE", True, True)
                        objCreditSales.UpdateDispatchTimeData(DeliveryPerson, Billno, clsAdmin.SiteCode)
    ...
    Public Function Themechange() As String
        ...
        Me.cmdRefreshGrid.Size = New System.Drawing.Size(35, 30)
        Me.cmdRefreshGrid.Text = ""
        Me.cmdRefreshGrid.VisualStyle = C1.Win.C1Input.VisualStyle.System
        Me.cmdRefreshGrid.Image = Nothing
        Me.cmdRefreshGrid.BackgroundImage = Global.Spectrum.My.Resources.Resources.Refresh_CSA2
        '  Me.cmdRefreshGrid.FlatStyle = FlatStyle.Flat
        Me.cmdRefreshGrid.BackgroundImageLayout = ImageLayout.Stretch
...
#End Region
    Sub PrintKOT(ByVal isKOTGenerateWithBill As Boolean)
        Try
            ...
            If grdCreditSales.Rows.Count > 1 Then
                ...
                Dim _billNo As String = CashMemoNo
    ...
    Private Sub cmdRefreshGrid_Click(sender As System.Object, e As System.EventArgs) Handles cmdRefreshGrid.Click
        ...
        'cmbOrderStatus.SelectedIndex = 3
        Dim str = ""
        str = SelectedDeliveryPartnerRadioBtn()
        BindCreditSales()
        If str.ToUpper() = "UBEREATS" Then
    ...
    Private Sub btnRejectOrder_Click(sender As Object, e As EventArgs) Handles btnRejectOrder.Click
        Try
            ...
            ShowMessage("do you wish to reject the order?", "Information", ResponsefromUser, "NO", "YES")

            If ResponsefromUser = 1 Then
                If clsDefaultConfiguration.EnableZomatoIntegration AndAlso (grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo").ToString().Substring(0, 2) = "ZM") Then
                    Dim OBjAR As New frmacceptReject
                    OBjAR.ZomatoRejectScreen = True
                    If (OBjAR.ShowDialog() <> Windows.Forms.DialogResult.Cancel) Then
                        Dim ZomatoRejectOrderJson = ZomatoRejectOrderAPIJson(grdCreditSales.Rows(grdCreditSales.Row)("OrderNo").ToString(), OBjAR.CboRejectReason.SelectedValue.ToString(), OBjAR.CtrlRejectionRemark.Text.ToString())
                        Dim JsonRejectOrderOutput As String = ""
                        If CallZomatoAPI(ZomatoRejectOrderJson, clsDefaultConfiguration.ZomatoRejectOrderAPI, clsDefaultConfiguration.ZomatoAPIKey, JsonRejectOrderOutput) Then
                            Dim JsonRejectOrderJsonObject As Dictionary(Of String, Object) = objCM.StringTojsonObject(JsonRejectOrderOutput)
                            If JsonRejectOrderJsonObject("status") = "success" Then
                                Dim CMD As String = "UPDATE DPACashMemoHdr set OrderStatus ='Reject' where billno= '" + grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo") + "';UPDATE DPACashMemoHdr set OrderStatus ='Reject' where billno= '" + grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo") + "'"
                                If objCM.InsertOrUpdateRecord(CMD.ToString()) = True Then
                                End If
                                If grdCreditSales.Row > 0 Then
                                    Dim Billno As String = grdCreditSales.Rows(grdCreditSales.Row)("BillNo")
                                    objCreditSales.UpdateHDCancelOrder(Billno, clsAdmin.SiteCode, True)
                                    ' Dim strOrderStatusId As String = objcomm.GetOrderStatusId("CANCELLED_BY_STORE", False)
                                    OrderStatusUpdates("CANCELLED_BY_STORE", False, True)
                                    selectedOrderStatusText = cmbOrderStatus.Text
                                    Call BindCreditSales()
                                    EnableDisabaledBottomButtons()
                                    '-- change method for updating deleted status - ashma
                                    ShowMessage(Billno & "Order Rejected Successfully", getValueByKey("CLAE05"))
                                End If
                            Else
                                ShowMessage(False, "Request Failed", "Zomato")
                            End If
                        Else
                            ShowMessage(False, "Request Failed", "Zomato")
                            Dim UpdateCmd As String = "UPDATE DPACashMemoReceipt set Status =0 where billno= '" + grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo") + "'"
                            objCM.InsertOrUpdateRecord(UpdateCmd)
                            cmdRefreshGrid_Click(Nothing, EventArgs.Empty)
                        End If
                    End If
                End If
            ...
            'If clsDefaultConfiguration.EnableZomatoIntegration Then
            '    Dim OBjAR As New frmacceptReject
            '    OBjAR.ZomatoRejectScreen = True
            '    If (OBjAR.ShowDialog() <> Windows.Forms.DialogResult.Cancel) Then
            '        Dim ZomatoAcceptOrderJson = ZomatoAcceptOrderAPIJson(grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo").ToString(), grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo").ToString(), OBjAR.TxtDeliveryTime.Text.ToString())
            ...
            '            webClient.Headers("X-Zomato-Api-Key") = clsDefaultConfiguration.ZomatoAPIKey
            '            '  reqString = Encoding.Default.GetBytes(JsonConvert.SerializeObject(dictData, Formatting.Indented))
            '            '   resByte = webClient.UploadData(clsDefaultConfiguration.ZomatoConformOrderAPI, "post", System.Text.Encoding.ASCII.GetBytes(ZomatoAcceptOrderJson))
            '            resByte = webClient.UploadData(clsDefaultConfiguration.ZomatoRejectOrderAPI, "post", System.Text.Encoding.ASCII.GetBytes(ZomatoAcceptOrderJson))
            '            resString = Encoding.Default.GetString(resByte)
            '            Console.WriteLine(resString)
            '            webClient.Dispose()
            '            Dim CMD As String = "UPDATE CashMemoHdr set OrderStatus ='Reject' where billno= '" + grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo") + "'"
            '            If objCM.InsertOrUpdateRecord(CMD.ToString()) = True Then
            ...
            'End If
            '**********************************************************************************
            'If clsDefaultConfiguration.EnableZomatoIntegration = False Then
            '    If grdCreditSales.Row > 0 Then
            '        Dim Billno As String = grdCreditSales.Rows(grdCreditSales.Row)("BillNo")
    ...
    Private Sub cmdAccept_Click(sender As Object, e As EventArgs) Handles cmdAccept.Click
        ...
        Try
            If grdCreditSales.Rows.Count < 2 Then Exit Sub
            Dim dt As New DataTable
            Dim CMD1 As String = "Select OrderStatus from DPACashMemoHdr where billno ='" + grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo") + "'"
            dt = objCM.GetFilledTable(CMD1)
            Dim status As String = dt.Rows(0)("OrderStatus").ToString()

            If status.ToString.ToUpper.Equals("VOID") Or status.ToString.ToUpper.Equals("VOIDNOTIFY") Then
                ShowMessage("Already order is rejected", getValueByKey("CLAE05"))
                selectedOrderStatusText = cmbOrderStatus.Text
                cmdRefreshGrid_Click(Nothing, EventArgs.Empty)
                Exit Sub
            ...
            If grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo").ToString().Substring(0, 2) = "UB" Then
                'Dim UberOrderToken As String = ""
                'Dim AcceptOrderOutput As String = ""
                ''GenerateUberEatsToken(UberOrderToken) If GenerateUberAcceptAPI(grdCreditSales.Rows(grdCreditSales.Row)("Orderno").ToString()) Then
                'If GenerateUberAcceptAPI(grdCreditSales.Rows(grdCreditSales.Row)("Orderno").ToString()) Then
                '    Dim CMD As String = "UPDATE DPACashMemoHdr set OrderStatus ='Accept' where billno= '" + grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo") + "'"
                '    objCM.InsertOrUpdateRecord(CMD)
                '    OrderStatusUpdates("ORDER_APPROVED_AT_STORE", False, True)
                '    ShowMessage("Order is  Accepted ", "Message")
                '    selectedOrderStatusText = cmbOrderStatus.Text
                '    cmdRefreshGrid_Click(Nothing, EventArgs.Empty)
                'End If
            Else                If clsDefaultConfiguration.EnableZomatoIntegration AndAlso (grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo").ToString().Substring(0, 2) = "ZM") Then
                    Dim OBjAR As New frmacceptReject
                    ' OBjAR.ZomatoAcceptScreen = True
                    ' If (OBjAR.ShowDialog() <> Windows.Forms.DialogResult.Cancel) Then
                    Dim ZomatoAcceptOrderJson = ZomatoAcceptOrderAPIJson(grdCreditSales.Rows(grdCreditSales.Row)("OrderNo").ToString(), grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo").ToString(), clsDefaultConfiguration.OrderAcceptReadyInMinutes)
                    ' Dim ZomatoAcceptOrderJson = ZomatoAcceptOrderAPIJson(grdCreditSales.Rows(grdCreditSales.Row)("OrderNo").ToString(), grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo").ToString(), OBjAR.TxtDeliveryTime.Text.ToString())
                    Dim JsonOutput As String = ""
                    If CallZomatoAPI(ZomatoAcceptOrderJson, clsDefaultConfiguration.ZomatoConformOrderAPI, clsDefaultConfiguration.ZomatoAPIKey, JsonOutput) Then
                        Dim objCm As New clsCommon
                        Dim JsonAcceptOrderJsonObject As Dictionary(Of String, Object) = objCm.StringTojsonObject(JsonOutput)
                        If JsonAcceptOrderJsonObject("status") = "success" Then
                            Dim CMD As String = "UPDATE DPACashMemoHdr set OrderStatus ='Accept' where billno= '" + grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo") + "'"
                            objCm.InsertOrUpdateRecord(CMD)
                            OrderStatusUpdates("ORDER_APPROVED_AT_STORE", False, True)
                            ShowMessage("Order is  Accepted ", "Message")
                            selectedOrderStatusText = cmbOrderStatus.Text
                            cmdRefreshGrid_Click(Nothing, EventArgs.Empty)
                            'objcomm.MooveOnlineOrdertoMainTables(clsAdmin.SiteCode, grdCreditSales.Rows(grdCreditSales.Row)("InvoiceNo").ToString())
                            'PrintKOT(False)
                            ''  CmdPrint_Click(Nothing, EventArgs.Empty)
                            'OrderStatusUpdates("ORDER_APPROVED_AT_STORE", False)
                            'selectedOrderStatusText = cmbOrderStatus.Text
    ...
    Private Sub CmdViewOrder_Click(sender As Object, e As EventArgs) Handles CmdViewOrder.Click
        ...
        End Try
    End Sub
    Sub OrderStatusUpdates(ByVal _statusType As String, Optional _isDispatch As Boolean = False, Optional IsCallFromZomato As Boolean = False)

        If grdCreditSales.Row > 0 Then
            Dim Billno As String = grdCreditSales.Rows(grdCreditSales.Row)("BillNo")
            Dim OrderstatusName = grdCreditSales.Rows(grdCreditSales.Row)("OrderStatus")
            Dim dtHD As New DataTable
            If IsCallFromZomato Then
                dtHD = objCreditSales.GetHomeDeliveryDataZomato(Billno, clsAdmin.SiteCode)
            Else
                dtHD = objCreditSales.GetHomeDeliveryData(Billno, clsAdmin.SiteCode)
            End If
    ...
    Private Sub cmbOrderStatus_SelectedValueChanged(sender As Object, e As System.EventArgs) Handles cmbOrderStatus.SelectedValueChanged
        ...
        End If
        Dim selectedDpstr As String = ""
        selectedDpstr = SelectedDeliveryPartnerRadioBtn()
        If selectedDpstr <> "" Then
            If selectedDpstr <> "All" Then
                'rowFilterString.Append(" and ")f
                rowFilterString.Append("DeliveryPartner='" & selectedDpstr & "'")
        ...
        End If
        If cmbOrderStatus.Text = "Dispatched" Or cmbOrderStatus.Text = "ALL" Or cmbOrderStatus.Text = "Rejected" Then

            Dim rowsfilterstring As String = "orderStatus = ' '"
            dtCreditSales.DefaultView.RowFilter = rowsfilterstring.ToString()
            grdCreditSales.DataSource = dtCreditSales.DefaultView
            EnableDisabaledBottomButtons()
        Else
    ...
    Private Sub lblToDeliveryDate_Click(sender As Object, e As EventArgs) Handles lblToDeliveryDate.Click

    End Sub
    Public Function ZomatoAcceptOrderAPIJson(ByVal order_id As String, ByVal external_order_id As String, ByVal delivery_time As String) As String
        Dim VoidJsonRequest As String = ""
        VoidJsonRequest = VoidJsonRequest + "{""order_id"":""" + order_id + ""","
        VoidJsonRequest = VoidJsonRequest + """external_order_id"":""" + external_order_id + ""","
        VoidJsonRequest = VoidJsonRequest + """delivery_time"":""" + delivery_time + """}"


        Return VoidJsonRequest

    End Function
    Public Function ZomatoRejectOrderAPIJson(ByVal order_id As String, ByVal rejection_message_id As String, ByVal vendor_rejection_message As String) As String
        Dim VoidJsonRequest As String = ""
        VoidJsonRequest = VoidJsonRequest + "{""order_id"":""" + order_id + ""","
        VoidJsonRequest = VoidJsonRequest + """rejection_message_id"":""" + rejection_message_id + ""","
        VoidJsonRequest = VoidJsonRequest + """vendor_rejection_message"":""" + vendor_rejection_message + """}"


        Return VoidJsonRequest

    End Function
    Public Function CallZomatoAPI(ByVal JsonString As String, ByVal APIUrl As String, ByVal Authkey As String, ByRef JsonOutput As String) As Boolean
        ' If clsDefaultConfiguration.EnableZomatoIntegration Then
        'Dim OBjAR As New frmacceptReject
        ''OBjAR.ZomatoRejectScreen = True
        '' If (OBjAR.ShowDialog() <> Windows.Forms.DialogResult.Cancel) Then
        ...
        'Try
        '    resByte = webClient.UploadData(APIUrl, "post", System.Text.Encoding.ASCII.GetBytes(JsonString))
        '    resString = Encoding.Default.GetString(resByte)
        '    Console.WriteLine(resString)
        '    webClient.Dispose()
        '    Return True
        ...
        Try
            ...
            writeLogForOnlineOrder(JsonString)
            Dim request As HttpWebRequest
            Dim response As HttpWebResponse
            request = CType(WebRequest.Create(APIUrl), HttpWebRequest)
            request.ContentType = "application/json"
            request.Headers("X-Zomato-Api-Key") = clsDefaultConfiguration.ZomatoAPIKey
            ...
            JsonOutput = New StreamReader(response.GetResponseStream()).ReadToEnd()
            writeLogForOnlineOrder(JsonOutput.ToString())
            request.Abort()
            Return True
        ...
        End Try
    End Function
    Public Function CallUberEatsAPI(ByVal OrderId As String, ByVal Authorization As String, ByVal Authkey As String, ByRef JsonOutput As String) As Boolean
        Try


            Dim request As HttpWebRequest
            Dim response As HttpWebResponse
            request = CType(WebRequest.Create("/v1/eats/orders/" + OrderId + "/accept_pos_order"), HttpWebRequest)
            request.Headers("Authorization") = "Bearer " + Authorization
            request.Headers("Contenet-Type") = "application/x-www-form-urlencoded"
            request.Headers("Scope") = "eats.order"
            request.Headers("grant_type") = "client_credentials"
            request.Method = "POST"
            request.AllowAutoRedirect = False
            Dim requeststream As Stream = request.GetRequestStream()
            Dim AcceptJsonstring = """" + "{" + """" + "reason" + """" + ":" + """" + "accepted" + "}"
            Dim postBytes As Byte() = System.Text.Encoding.ASCII.GetBytes(AcceptJsonstring)
            requeststream.Write(postBytes, 0, postBytes.Length)
            requeststream.Close()
            response = CType(request.GetResponse(), HttpWebResponse)
            JsonOutput = New StreamReader(response.GetResponseStream()).ReadToEnd()
        ...
        End Try
    End Function
    Public Function GenerateUberEatsToken(ByVal OrderId As String, ByRef JsonOutput As String) As Boolean
        Try


            'Dim request As HttpWebRequest
            'Dim response As HttpWebResponse
            'request = CType(WebRequest.Create("https://login.uber.com/oauth/v2/token"), HttpWebRequest)
            'request.Headers("Contenet-Type") = "application/x-www-form-urlencoded"
            'request.Method = "POST"
            'request.AllowAutoRedirect = False
            'Dim requeststream As Stream = request.GetRequestStream()
            'Dim postData = "clinet_id=" + clsDefaultConfiguration.UberClinetId + "&client_secret=" + clsDefaultConfiguration.Uberclientsecret + "&grant_type=" + "client_credentials" + "&scope=" + "eats.order"""
            ...
            'request.AddHeader("Authorization", "Bearer " & UberEatsOrderAccesToekn)
            'request.AddHeader("Content-Type", "application/x-www-form-urlencoded")
            'request.AddHeader("grant_type", grant_type)
            'request.AddHeader("scope", Orderscope)
        ...
        End Try
    End Function
    Public Function CallUberAcceptAPI(ByVal OrderId As String, ByVal Authorization As String, ByRef AcceptOrderOutput As String) As Boolean
        Try


            Dim request As HttpWebRequest
            Dim response As HttpWebResponse
            request = CType(WebRequest.Create(clsDefaultConfiguration.UberOrderAcceptAPI.Replace("OrderId", OrderId)), HttpWebRequest)
            request.Headers("Authorization") = "Bearer " + Authorization
            request.Headers("Contenet-Type") = "application/x-www-form-urlencoded"
            request.Headers("Scope") = "eats.order"
            request.Headers("grant_type") = "client_credentials"
            ...
            requeststream.Close()
            response = CType(request.GetResponse(), HttpWebResponse)
            AcceptOrderOutput = New StreamReader(response.GetResponseStream()).ReadToEnd()
            request.Abort()
            Return True
        Catch ex As Exception
            LogException(ex)
        ...
        End Try
    End Function
    Public Function GenerateUberEatsToken(ByRef UberOrderToken As String) As Boolean
        Try

            Dim request As HttpWebRequest
            Dim response As HttpWebResponse
            request = CType(WebRequest.Create(clsDefaultConfiguration.UberGenerateTokenAPI), HttpWebRequest)
            request.ContentType = "application/json"
            request.Headers("ContentType") = "application/json"
            request.Method = "GET"
            request.AllowAutoRedirect = False
            response = CType(request.GetResponse(), HttpWebResponse)
            Dim JsonOut = New StreamReader(response.GetResponseStream()).ReadToEnd()
            Dim objCm As New clsCommon
            Dim JsonAcceptOrderJsonObject As Dictionary(Of String, Object) = objCm.StringTojsonObject(JsonOut)
            UberOrderToken = JsonAcceptOrderJsonObject("access_token")
    ...
    Public Function GenerateUberAcceptAPI(ByRef orderno As String) As Boolean
        Try

            Dim request As HttpWebRequest
            Dim response As HttpWebResponse

            'Dim urll As String = "https://localhost:15146/UberEats.svc/GetAcceptorderOfUber/" & orderno
            'Dim urll As String = "https://deliverypartner.criti.in/UberEats.svc/GetAcceptorderOfUber/" & orderno
            Dim urll As String = clsDefaultConfiguration.UberOrderAcceptAPI & orderno
            request = CType(WebRequest.Create(urll), HttpWebRequest)
            request.ContentType = "application/json"
            request.Headers("ContentType") = "application/json"
            request.Method = "GET"
            request.AllowAutoRedirect = False
    ...
    End Function
    Private Sub writeLogForOnlineOrder(ByVal mes As String)
        Dim ax As New ApplicationException(mes)
        LogException(ax)
    End Sub

    Function SelectedDeliveryPartnerRadioBtn() As String
        Try
            For i = 0 To TableLayoutPanel3.RowCount - 1
                For j = 0 To TableLayoutPanel3.ColumnCount - 1
                    Dim Control = TableLayoutPanel3.GetControlFromPosition(j, i)
                    If Not Control Is Nothing Then
                        Dim btn As RadioButton = DirectCast(Control, RadioButton)
    ...
    Private Sub SelectUberEatsByDefault()
        Try

            Dim isUberEatsExit As Boolean = False
            ...
            For i = 0 To TableLayoutPanel3.RowCount - 1
                For j = 0 To TableLayoutPanel3.ColumnCount - 1
                    ...
                    If Not Control Is Nothing Then
                        ...
                        If btn.Text.ToUpper.ToString.Equals("UBEREATS") Then
                            btn.Checked = True
                            isUberEatsExit = True
                        End If
            ...
            If isUberEatsExit = False Then
                For i = 0 To TableLayoutPanel3.RowCount - 1
                    For j = 0 To TableLayoutPanel3.ColumnCount - 1
                        Dim Control = TableLayoutPanel3.GetControlFromPosition(j, i)
                        If Not Control Is Nothing Then
                            Dim btn As RadioButton = DirectCast(Control, RadioButton)
                            If btn.Text.ToUpper.ToString.Equals("ALL") Then
                                btn.Checked = True
                            End If
                        End If
                    Next
        ...
        End Try
    End Sub
    Public Sub CreateDynamicRadioBtn(ByVal dt As DataTable)
        Try

            If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then

                dt.DefaultView.Sort = "DelieveryPartnerName ASC"
                dt = dt.DefaultView.ToTable()

                Dim count As Integer = 0
                For Each dr In dt.Rows

                    Dim rdiBtn As New RadioButton
                    rdiBtn.Text = dr("DelieveryPartnerName")
                    rdiBtn.Name = dr("DelieveryPartnerId")
                    rdiBtn.Dock = DockStyle.None
                    rdiBtn.BackColor = Color.WhiteSmoke
                    rdiBtn.ForeColor = Color.Black
                    rdiBtn.Font = New Font("Neo Sans", 9, FontStyle.Bold)
                    rdiBtn.BringToFront()
                    rdiBtn.Image = Nothing
                    rdiBtn.ImageAlign = System.Drawing.ContentAlignment.TopCenter
                    rdiBtn.FlatStyle = FlatStyle.Flat
                    rdiBtn.FlatAppearance.BorderSize = 1
                    rdiBtn.Size = New System.Drawing.Size(100, 100)
                    AddHandler rdiBtn.CheckedChanged, AddressOf DynamicBtnClick
                    If count <= 4 Then
                        TableLayoutPanel3.Controls.Add(rdiBtn, count, 0)
                    Else
                        TableLayoutPanel3.Controls.Add(rdiBtn, count - 5, 1)
                    End If
    ...
    Private Sub DynamicBtnClick(sender As System.Object, e As System.EventArgs)
        ...
        Dim rbtnFilter As String = ""


        rbtnFilter = rbtn.Text
        If rbtnFilter.ToUpper.ToString.Equals("UBEREATS") Then
            cmbOrderStatus.SelectedValue = "Accepted"
            cmbOrderStatus.SelectedIndex = 2
        Else
            If rbtnFilter.ToUpper.ToString.Equals("ALL") Then
                cmbOrderStatus.SelectedValue = "ALL"
                cmbOrderStatus.SelectedIndex = 4
            Else
                cmbOrderStatus.SelectedValue = "OPEN"
                cmbOrderStatus.SelectedIndex = 3
            End If
