...
Public Class CreateNewCustomer
    Inherits CreateNewCustomerBase
    Implements ICreateNewCustomer
    Private objClsCommon As New clsCommon
    Public Function GetCustomerInfo(ByVal clpProgramId As String, ByVal PrimaryKeyNo As String, ByVal KeyIsMobileNo As Boolean) As GetCustomerResponse Implements ICreateNewCustomer.GetCustomerInfo
        Try
            Dim response As New GetCustomerResponse
            Dim query = String.Empty
            ' ---Changed By Mahesh :: Remove mobile no filter to card no as par Rama suggestion
            query = "select CardNo, ClpProgramId, AccountNo, SiteCode, FirstName, MiddleName, SurName, Gender, BirthDate, EmailId,  Mobileno, TotalBalancePoint,CustomerGroup,resgistrationstatus,CardType, Res_Phone AS ResidenceNumber " & _
                    " from CLPCustomers " & _
    ...
    Public Function GetCustomerMaster() As GetCustomerMasterResponse Implements ICreateNewCustomer.GetCustomerMaster
        Try
    ...
    Public Function SaveCustomerInfo(ByRef request As SaveCustomerRequest, Optional ByVal dtDeleteAddress As DataTable = Nothing, Optional ByVal dtDeleteContact As DataTable = Nothing) As Boolean Implements ICreateNewCustomer.SaveCustomerInfo
        Dim tran As SqlTransaction
        Try
            Dim query As String = String.Empty
            OpenConnection()
            ...
            If CheckIfInsertOrUpdate(request) = False Then
                ...
                With request.CLPCustomer
                    ...
                    query = "insert into dbo.CLPCustomers (CardNo, ClpProgramId, AccountNo, SiteCode, " & _
                                    " Title, FirstName, MiddleName, SurName, " & _
                                     ...
                                     " TotalBalancePoint, PointsAccumlated, PointsRedeemed, " & _
                                     " CREATEDAT, CREATEDBY, CREATEDON, UPDATEDAT, UPDATEDBY, UPDATEDON, STATUS," & _
                                     " OldCardType,CustomerGroup,resgistrationstatus,CompanyName)" & _
                            " Values('" & .CardNumber & "','" & .ClpProgId & "','" & .CardNumber & "','" & .SiteCode & "'," & _
                                        "'" & .Title & "','" & .FirstName & "','" & .MiddleName & "','" & .LastName & "'," & _
                                        "'" & String.Format("{0} {1}", .FirstName, .LastName) & "','" & .CardType & "','" & .Gender & "'," & IIf(.BirthDate = DateTime.MinValue, "null", "'" & .BirthDate.ToString("yyyy-MM-dd") & "'") & "," & _
                                        "'" & .Education & "','" & .Occupation & "','" & .EmailId & "','" & .MaritalStatus & "'," & _
                                        ...
                                        & .BalancePoints & ",0.0,0.0," & _
                                        "'" & .SiteCode & "','" & request.CreatedBy & "',getdate(),'" & .SiteCode & "','" & request.CreatedBy & "',getdate(),1,'','" & .CustomerGroup & "','" & .RegistrationStatus & "','" & .CompanyName & "') "
                ...
                For Each address In request.CLPCustomer.AddressList
                    ...
                    query = " INSERT INTO CLPCustomerAddress (CardNo, Clpprogramid, AddressType, AddressLn1, AddressLn2, AddressLn3, AddressLn4, PinCode, CityCode,StateCode,CountryCode, " & _
                  "   Defaults, CREATEDAT, CREATEDBY, CREATEDON, UPDATEDAT, UPDATEDBY, UPDATEDON, STATUS,DefaultAddress,Department,SrNo )" & _
                  "    values('" & address.CardNumber & "','" & address.ClpProgId & "','" & address.AddressType & "','" & address.AddLine1 & "','" & address.AddLine2 & "','" & address.AddLine3 & "','" & _
                   address.AddLine4 & "','" & address.PinCode & "','" & address.City & "','" & address.State & "','" & address.Country & "'," & DefaultValue & ",'" & request.CLPCustomer.SiteCode & "'," & _
                   "'" & request.CreatedBy & "',getdate(),'" & request.CLPCustomer.SiteCode & "','" & request.CreatedBy & "',getdate(),1," & address.DefaultAddress & ",'" & address.Department & "'," & srnum & ") "
                ...
                '----ContactDetails
                With request.CLPCustomerContact
                    srnum = 1
                    For Each contact In request.CLPCustomer.ContactList
                        query = "insert into dbo.CLPCustomerContacts (CardNo, ClpProgramId, SiteCode, " & _
                                        " ContactType,ContactValue, " & _
                                         " CREATEDAT, CREATEDBY, CREATEDON, UPDATEDAT, UPDATEDBY, UPDATEDON, STATUS,SrNo)" & _
                                        " Values('" & request.CLPCustomer.CardNumber & "','" & request.CLPCustomer.ClpProgId & "','" & request.CLPCustomer.SiteCode & "'," & _
                                             "'" & contact.ContactType & "','" & contact.ContactValue & "'," & _
                                            "'" & request.CLPCustomer.SiteCode & "','" & request.CreatedBy & "',getdate(),'" & request.CLPCustomer.SiteCode & "','" & request.CreatedBy & "',getdate(),1," & srnum & ")"
            ...
            Else
                ...
                With request.CLPCustomer
                    ...
                    query += "UPDATEDBY = '" & request.UpdatedBy & "' , UPDATEDON = getdate() , resgistrationstatus='" & .RegistrationStatus & "', "
                    query += "TotalBalancePoint = '" & .BalancePoints & "', "
                ...
                query = " SELECT IsNull(Max(SrNo),0) As SrNo FROM  CLPCustomerAddress " & _
                            " WHERE CardNo = '" & request.CLPCustomer.CardNumber & "' and Clpprogramid = '" & request.CLPCustomer.ClpProgId & "'"

                Dim max = GetScalerValue(query, tran)
                ...
                For Each address In request.CLPCustomer.AddressList
                    ...
                    query = " SELECT 1 FROM  CLPCustomerAddress " & _
                            " WHERE CardNo = '" & request.CLPCustomer.CardNumber & "' and Clpprogramid = '" & request.CLPCustomer.ClpProgId & "' and AddressType ='" & address.AddressType & "'and SrNo ='" & address.SrNo & "'"
                    ...
                    If dtTemp <> Nothing AndAlso dtTemp.ToString() = "1" Then
                        query = "update CLPCustomerAddress Set AddressLn1 = '" & address.AddLine1 & "',AddressLn2='" & address.AddLine2 & "', AddressLn3='" & address.AddLine3 & "', AddressLn4='" & address.AddLine4 & "',PinCode='" & address.PinCode & "'," & _
                                "CityCode='" & address.City & "',StateCode='" & address.State & "',CountryCode='" & address.Country & "', UPDATEDAT = '" & request.CLPCustomer.SiteCode & "', UPDATEDBY = '" & request.UpdatedBy & "' , UPDATEDON = getdate() ," & _
                                "DefaultAddress='" & address.DefaultAddress & "', Department='" & address.Department & "',SrNo='" & address.SrNo & "'" & _
                                " where CardNo = '" & request.CLPCustomer.CardNumber & "' and Clpprogramid = '" & request.CLPCustomer.ClpProgId & "' and AddressType ='" & address.AddressType & "'and SrNo ='" & address.SrNo & "'"
                    ...
                    Else
                       ...
                       "    values('" & request.CLPCustomer.CardNumber & "','" & request.CLPCustomer.ClpProgId & "','" & address.AddressType & "','" & address.AddLine1 & "','" & address.AddLine2 & "','" & address.AddLine3 & "','" & _
                        address.AddLine4 & "','" & address.PinCode & "','" & address.City & "','" & address.State & "','" & address.Country & "'," & DefaultValue & ",'" & request.CLPCustomer.SiteCode & "'," & _
                        "'" & request.CreatedBy & "',getdate(),'" & request.CLPCustomer.SiteCode & "','" & request.CreatedBy & "',getdate(),1," & address.DefaultAddress & ",'" & address.Department & "'," & max & ") "
                ...
                '-----For soft Delete Adreeses from table
                If dtDeleteAddress.Rows.Count > 0 Then
                    For index = 0 To dtDeleteAddress.Rows.Count - 1
                        query = "update CLPCustomerAddress Set status=0,UPDATEDON = getdate()" & _
                                   "where CardNo = '" & request.CLPCustomer.CardNumber & "' and Clpprogramid = '" & request.CLPCustomer.ClpProgId & "' and SrNo ='" & dtDeleteAddress.Rows(index)("SrNo") & "'"
                        InsertOrUpdateRecord(query, tran)
                    Next
                ...
                query = " SELECT IsNull(Max(SrNo),0) As SrNo FROM  CLPCustomerContacts " & _
                           " WHERE CardNo = '" & request.CLPCustomer.CardNumber & "' and Clpprogramid = '" & request.CLPCustomer.ClpProgId & "'"
                Dim maxcontact = GetScalerValue(query, tran)


                For Each contact In request.CLPCustomer.ContactList
                    query = " SELECT 1 FROM  CLPCustomerContacts " & _
                            " WHERE CardNo = '" & request.CLPCustomer.CardNumber & "' and Clpprogramid = '" & request.CLPCustomer.ClpProgId & "' and SrNo ='" & contact.SrNo & "'"
                    Dim dtTemp = GetScalerValue(query, tran)

                    If dtTemp <> Nothing AndAlso dtTemp.ToString() = "1" Then
                        query = "update CLPCustomerContacts Set ContactType = '" & contact.ContactType & "',ContactValue='" & contact.ContactValue & "',UPDATEDON=getdate()" & _
                               " where CardNo = '" & request.CLPCustomer.CardNumber & "' and Clpprogramid = '" & request.CLPCustomer.ClpProgId & "' and SrNo ='" & contact.SrNo & "'"
                    ...
                    Else
                        ...
                        query = "insert into dbo.CLPCustomerContacts (CardNo, ClpProgramId, SiteCode, " & _
                                    ...
                                    " Values('" & request.CLPCustomer.CardNumber & "','" & request.CLPCustomer.ClpProgId & "','" & request.CLPCustomer.SiteCode & "'," & _
                                         "'" & contact.ContactType & "','" & contact.ContactValue & "'," & _
                                        "'" & request.CLPCustomer.SiteCode & "','" & request.CreatedBy & "',getdate(),'" & request.CLPCustomer.SiteCode & "','" & request.CreatedBy & "',getdate(),1," & maxcontact & ")"
                ...
                '-----For soft Delete Adreeses from table
                If dtDeleteContact.Rows.Count > 0 Then
                    For index = 0 To dtDeleteContact.Rows.Count - 1
                        query = "update CLPCustomerContacts Set status=0,UPDATEDON = getdate()" & _
                                   "where CardNo = '" & request.CLPCustomer.CardNumber & "' and Clpprogramid = '" & request.CLPCustomer.ClpProgId & "' and SrNo ='" & dtDeleteContact.Rows(index)("SrNumber") & "'"
                        InsertOrUpdateRecord(query, tran)
                    Next
            ...
            'With request.CLPCustomerContact
            '    If CheckIfInsertOrUpdate(request) = True Then
            '        query = "delete from dbo.CLPCustomerContacts where CardNo='" & request.CLPCustomer.CardNumber & "' And ClpProgramId='" & request.CLPCustomer.ClpProgId & "' And SiteCode='" & request.CLPCustomer.SiteCode & "';"
            '        InsertOrUpdateRecord(query, tran)
            '    End If
            ...
            '                        " Values('" & request.CLPCustomer.CardNumber & "','" & request.CLPCustomer.ClpProgId & "','" & request.CLPCustomer.SiteCode & "'," & _
            '                             "'" & contact.ContactType & "','" & contact.ContactValue & "'," & _
            '                            "'" & request.CLPCustomer.SiteCode & "','" & request.CreatedBy & "',getdate(),'" & request.CLPCustomer.SiteCode & "','" & request.CreatedBy & "',getdate(),1)"
