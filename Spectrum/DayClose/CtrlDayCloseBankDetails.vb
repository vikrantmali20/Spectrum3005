...
Public Class CtrlDayCloseBankDetails
    ...
    Dim JKSalesReconciliationPath As String = ""
    Dim ObjC As New clsCommon
    Public Shared SyncTimer As New Timer()
    Dim IsSyncComplete As Boolean = False
    Dim IsSyncStart As Boolean = False
    Dim IsSyncAlreadyWaitForThreeMinutes As Boolean = False
    Public Delegate Sub Temp1()
    Dim SkipSync As Boolean = False
    ...
    Public Property DayCloseResponse As DayCloseBankDetailsResponse
        ...
        End Set
    End Property
    Public _FormCloseByUserBankDtl As Boolean = True
    Public Property FormCloseByUserBankDtl As Boolean
        Get
            Return _FormCloseByUserBankDtl
        End Get
        Set(ByVal value As Boolean)
            _FormCloseByUserBankDtl = value
        End Set
    End Property
    ...
    Private Sub CtrlDayCloseBankDetails_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If clsDefaultConfiguration.ThemeSelect = "Theme 1" Then
                Themechange()
            End If
            AddHandler SyncTimer.Tick, AddressOf SyncTimerClick
            IsSyncAlreadyWaitForThreeMinutes = False
            SyncTimer.Interval = 180000
    ...
    Public Sub GetDayCloseBankDetailsData(ByRef request As DayCloseBankDetailsRequest)
        Try
            If CashDetailsList Is Nothing OrElse CashDetailsList.Count = 0 Then
                Dim response As DayCloseBankDetailsResponse = Instance.GetDayCloseBankDetails(request, clsDefaultConfiguration.AutoPopulateQtyForAllDayCloseScreens)
    ...
    Private Sub btnDayClose_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDayClose.Click
        writeDaycloseLog("User Click on dayclose button")
        ...
        '        End Try
    End Sub
    Public Sub ChangeVisibility(ByVal steps As Integer, Optional ByVal IsSyncInProgress As Boolean = False)
        If steps = 0 Then
            ' waitPopupMsg = DayCloseSyncProgress("Day Close and Synchronization in Progress. Please wait..", getValueByKey("CLAE04"))
            If IsSyncInProgress = True Then
                If IsSyncAlreadyWaitForThreeMinutes = True Then
                    waitPopupMsg.Close()
                    waitPopupMsg = DayCloseSyncProgress("Network is very poor, please wait for Synchronization to be completed.", getValueByKey("CLAE04"))
                    If SyncTimer.Enabled Then
                        SyncTimer.Stop()
                    End If
                Else
                    waitPopupMsg.Close()
                    waitPopupMsg = DayCloseSyncProgress("Network connectivity is taking time, so please wait for 3 more minute's.", getValueByKey("CLAE04"))

                End If
            Else
                waitPopupMsg = DayCloseSyncProgress("Day Close and Synchronization in Progress. Please wait for atleast 3 minute's.", getValueByKey("CLAE04"))
    ...
    Private Sub bw_DoWork(sender As Object, e As DoWorkEventArgs)
        Try
                ...
                Try
                    ...
                    If dtInfo IsNot Nothing Then
                        ...
                        Catch ex As Exception
                            ...
                            'ShowMessage("Weak or No Internet Connection!", getValueByKey("CLAE04"))
                            'Exit Sub
                            If clsDefaultConfiguration.AllowDayCloseInOfflineMode = True Then 'vipin 17.04.2018
                                Dim result As Integer = MessageBox.Show("There is not internet and hence sync cannot be performed. Do you still want to perform day close in offline mode?", "Day Close", MessageBoxButtons.YesNo)
                                If result = DialogResult.Yes Then
                                    SkipSync = True
                                    GoTo ContinueDayCloseInOfflineMode
                                Else
                                    ShowMessage("Weak or No Internet Connection!", getValueByKey("CLAE04"))
                                    Exit Sub
                                End If
                        ...
                        If dtInfo.Rows.Count > 0 Then
                            ...
                            Catch ex As Exception
                                ...
                                'Exit Sub
                                If clsDefaultConfiguration.AllowDayCloseInOfflineMode = True Then 'vipin 17.04.2018
                                    Dim result As Integer = MessageBox.Show("Server Down and hence sync cannot be performed. Do you still want to perform day close in offline mode?", "Day Close", MessageBoxButtons.YesNo)
                                    If result = DialogResult.Yes Then
                                        SkipSync = True
                                        GoTo ContinueDayCloseInOfflineMode
                                    Else
                                ...
                                Try
                                    ...
                                    Using listener As New Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp)
                                        ...
                                        Try
                                            ...
                                            'code added by vipul for calling sync file maximum 10 times if responce is less than zero
                                            Dim isProcessStarted As Boolean = False

                                            For index = 1 To 10
                                                Dim pid = Shell(Application.StartupPath & "\run_dayCloseSynch.bat", AppWinStyle.Hide)
                                                writeDaycloseLog("current iteration of run_dayCloseSynch.bat :" + Convert.ToString(index) + " output of pid:" + Convert.ToString(pid))
                                                If pid > 0 Then
                                                    isProcessStarted = True
                                                    Exit For
                                                End If
                                            Next
                                            If isProcessStarted = False Then
                                                ' do all the needful cleanup
                                                Exit Sub
                                            End If
            ...
            ''Synch on Day Close End
ContinueDayCloseInOfflineMode: writeDaycloseLog("started Day Close ")
            Dim request As New BankDetailsSaveDataRequest
            ...
            If Instance.SaveBankDetailsData(request, clsDefaultConfiguration.DayCloseReportPath, clsDefaultConfiguration.UpdateStockOnDayCloseWastage) Then
                ...
                If clsDefaultConfiguration.SyncOnDayClose AndAlso SkipSync = False Then
                    Dim client As New WebClient()
                    client.Headers("Content-type") = "application/json"
                    '-- invoke the REST method
                    Dim data As Byte()
                    Try
                        ...
                        'Dim url As String = "http://" & IpAddr & ":" & port & "/posSeam/rest/dayclose/update?siteCode=" & clsAdmin.SiteCode & "&openDate=" & clsAdmin.DayOpenDate.ToString("yyyy-MM-dd")
                        'added by khusrao adil on 16-02-2018 for JK requirement .added new parameters
                        'request.UpdatedOn
                        'request.CreatedOn
                        'Dim url As String = "http://" & IpAddr & ":" & port & "/posSeam/rest/dayclose/update?siteCode=" & clsAdmin.SiteCode & "&openDate=" & clsAdmin.DayOpenDate.ToString("yyyy-MM-dd") & "&FinYear=" & clsAdmin.Financialyear & "&CreatedBy=" & clsAdmin.UserCode & "&UpdatedBy=" & clsAdmin.UserCode
                        Dim url As String = "http://" & IpAddr & ":" & port & "/posSeam/rest/dayclose/update?siteCode=" & clsAdmin.SiteCode & "&openDate=" & clsAdmin.DayOpenDate.ToString("yyyy-MM-dd") & "&FinYear=" & clsAdmin.Financialyear & "&CreatedBy=" & _
                           request.CreatedBy & "&UpdatedBy=" & clsAdmin.UserCode & _
                            "&CreatedOn=" & request.CreatedOn.ToString("yyyy-MM-dd HH:mm:ss.fff") & _
                            "&UpdatedOn=" & request.UpdatedOn.ToString("yyyy-MM-dd HH:mm:ss.fff")
                        'yyyy-MM-dd HH:mm:ss.fff
                        writeDaycloseLog(url)
                ...
                End If
                Call Instance.ResetGLNoRangeObjects("SpectrumMattler", clsAdmin.SiteCode) 'added by vipin For new Spectrum Mettler Reset flag 14-03-2018
                writeDaycloseLog("Insert data in day ArticleStockDayCloseBalances")
                Call Instance.SaveDayCloseArticleStockBalance(clsAdmin.DayOpenDate, request.CreatedAt, request.CreatedBy)
                If DayClosed IsNot Nothing Then
                    DayClosed()
                End If
                ...
                If clsDefaultConfiguration.ClientForMail = "JK" Then
                    ...
                    Call GenerateJKSalesReconciliation(clsAdmin.SiteCode, clsAdmin.DayOpenDate, clsAdmin.DayOpenDate)
                    Call GenerateDSRReport(id)
                    Dim objReport As New DayCloseReportController
                    writeDaycloseLog("generating day close report")
                    objReport.GenerateDayCloseReport(New DayCloseReportModel With {.SiteCode = request.SiteCode, .ToDate = request.DayCloseDate}, clsDefaultConfiguration.DayCloseReportPath)
                ElseIf clsDefaultConfiguration.ClientForMail = "PC" Then
                    Call GenerateDayCloseReport(id)
                Else
                ...
                If clsDefaultConfiguration.IsMailSend Then
                    If clsDefaultConfiguration.ClientForMail = "Spectrum" Then
                        ...
                        'added by adil on 07-03-2018 for blank report generete and mail sent issue-2623
                        path = DayCloseReportController.PathForEmail
                        If Not String.IsNullOrEmpty(path) Then
                            Call SendMail(path, id, True)
                        End If
                    End If
                End If
                ...
                End If
                Me.ParentForm.Enabled = False 'vipin 
                ' ShowMessage(String.Format(getValueByKey("dayclosesuccessmsg"), clsAdmin.DayOpenDate.ToShortDateString()), getValueByKey("CLAE04"))
                DayClosePopUp(String.Format(getValueByKey("dayclosesuccessmsg"), clsAdmin.DayOpenDate.ToShortDateString()), getValueByKey("CLAE04"))
                'Me.Invoke(ChangePictureVisibility, 9)
                Me.ParentForm.Enabled = True 'vipin 
                DisableTransactionMainMenu(False)
                MDISpectrum.DayOpenMenuItem.Enabled = True
            ...
            Else
                ...
                Me.ParentForm.Enabled = False 'vipin
                ' ShowMessage((getValueByKey("daycloseerrormsg")), getValueByKey("CLAE05"))
                DayClosePopUp(String.Format(getValueByKey("daycloseerrormsg")), getValueByKey("CLAE05"))
            ...
            If clsDefaultConfiguration.ClientForMail = "JK" Then
                Try
                    'No need to call this Report path again .This was called in the GenerateDayCloseReport vipin 16.02.2018
                    If Not String.IsNullOrEmpty(path) Then
                        Process.Start(path)
                    End If
                    If Not String.IsNullOrEmpty(JKDayOfReporthPath) Then
                        Process.Start(JKDayOfReporthPath)
                    End If
                    If Not String.IsNullOrEmpty(JKProductMixReportPath) Then
                        Process.Start(JKProductMixReportPath)
                    End If
                    If Not String.IsNullOrEmpty(JKSalesReconciliationPath) Then
                        Process.Start(JKSalesReconciliationPath)
                    End If
                Catch ex As Exception
                    LogException(ex)
                End Try
                Me.FormCloseByUserBankDtl = False
                writeDaycloseLog("Application close")
                Application.Exit()
            End If
            'Me.ParentForm.Close()
        Catch ex As Exception
    ...
    Public Sub DoStuff()
        ...
        '
    End Sub
    Private Function SendMail(ByVal Path As String, ByVal Idlist As String, Optional ByVal MaintainMailSendStatus As Boolean = False)
        For index = 0 To 4
            Try
                DoStuff()
                Dim BccMailId As String = ""  'added by khusrao adil on 07-02-2018 Jk requirement
                Dim readerPath As New StreamReader(Path)
                Dim readerJKDayOf As StreamReader
                Dim readerJKProductMix As StreamReader
                Dim rederJKSalesReconciliation As StreamReader
                If clsDefaultConfiguration.ClientForMail = "JK" Then
                    ...
                    End If
                    If JKSalesReconciliationPath <> "" Then
                        rederJKSalesReconciliation = New StreamReader(JKSalesReconciliationPath)
                    End If
                ...
                Using reasder As New StreamReader(Path)
                    ...
                    For Each row In dt.Rows
                        ...
                        ElseIf row("FldLabel").ToString() = "SMTP.UserName" Then
                            ...
                            BccMailId = row("FldValue").ToString() 'added by khusrao adil on 07-02-2018 Jk requirement
                    ...
                    mm.Bcc.Add(New MailAddress(BccMailId)) 'added by khusrao adil on 07-02-2018 Jk requirement
                    ...
                    'added by khusrao adil on 10-09-2017 for sending back office link in mail for jk
                    If clsDefaultConfiguration.ClientForMail = "JK" Then
                        Dim BOLink = clsDefaultConfiguration.OpenBackOfficeFromFO
                        If BOLink <> "" Then
                            msg.Append(vbCrLf + vbCrLf)
                            Dim BOLinkText = "Back Office Link:-  " & BOLink & ""
                            msg.Append(BOLinkText + vbCrLf)
                            msg.Append(vbCrLf + vbCrLf)
                        End If
                    End If
                    ...
                    If clsDefaultConfiguration.ClientForMail = "JK" Then
                        ...
                        End If
                        If JKSalesReconciliationPath <> "" Then
                            mm.Attachments.Add(New Attachment(rederJKSalesReconciliation.BaseStream, JKSalesReconciliationPath))
                        End If
                    End If
                    mm.IsBodyHtml = False
                    ...
                    ' smtp.Port = 587
                    smtp.Send(mm)
                    If MaintainMailSendStatus = True Then
                        ObjC.FoDayCloseMailSend(clsAdmin.SiteCode, clsAdmin.UserCode, 1, clsAdmin.DayOpenDate)
                    End If
                    Exit For
                End Using
            Catch ex As Exception
                If MaintainMailSendStatus = True Then
                    ObjC.FoDayCloseMailSend(clsAdmin.SiteCode, clsAdmin.UserCode, 0, clsAdmin.DayOpenDate)
                End If
                LogException(ex)               
            End Try
        Next
    ...
    '' Added By ketan JK SalesReconciliation Report
    Private Function GenerateJKSalesReconciliation(ByVal SiteCode As String, ByVal FromDate As Date, ByVal ToDate As Date)
        Try
            Dim DSSalesReconciliation As DataSet
            Dim clsObj As New clsSalesReconciliation
            DSSalesReconciliation = clsObj.SalesReconciliationReport(SiteCode, FromDate, ToDate)
            Dim reportViewer2 As New ReportViewer()
            ...
            Dim mybytes As [Byte]() = reportViewer2.LocalReport.Render("Excel")
            'Byte[] mybytes = report.Render("PDF"); for exporting to PDF
            JKSalesReconciliationPath = clsDefaultConfiguration.DayCloseReportPath & "\JKSalesReconciliationReport_" & DateTime.Now.ToString("dd-MM-yyyy-hhmmss") & ".xls"
            Using fs As FileStream = File.Create(JKSalesReconciliationPath)
                fs.Write(mybytes, 0, mybytes.Length)
            End Using
        Catch ex As Exception
            LogException(ex)
    ...
    Private Sub dgChequeDetails_CellBeginEdit(sender As Object, e As DataGridViewCellCancelEventArgs) Handles dgChequeDetails.CellBeginEdit
        If clsDefaultConfiguration.AllowOnScreenKeyBoard Then
            OnTouchKeyBoard()
        End If
    End Sub
    'code added by vipul for dayclose pop up issue
    Private Sub DayClosePopUp(Optional ByVal DayClosePopMsg As String = "", Optional ByVal DayClosePopHdr As String = "")
        Dim objMess = New frmSpecialPrompt(DayClosePopHdr)
        objMess.lblMessage.Font = New Font("Arial", 12.5, FontStyle.Bold)
        objMess.lblMessage.Text = DayClosePopMsg
        objMess.ShowMessage = True
        objMess.TopMost = False
        objMess.AcceptButton = objMess.cmdOk
        objMess.ShowDialog(Me)
    End Sub
    Private Sub SyncTimerClick() 'vipul

        Try
            If IsSyncComplete = False And IsSyncStart = True Then
                SyncTimer.Stop()
                ChangeVisibility(0, True)
                SyncTimer.Interval = 18000
                SyncTimer.Start()
                If IsSyncAlreadyWaitForThreeMinutes = False Then
                    IsSyncAlreadyWaitForThreeMinutes = True
                End If
            End If
        Catch ex As Exception
            LogException(ex)
