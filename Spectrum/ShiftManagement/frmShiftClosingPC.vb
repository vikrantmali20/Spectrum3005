...
Imports System.Text
Public Class frmShiftClosingPC
    Public Shared _CardAmt As String   'vipin
    Public Shared Property CardAmt() As String
        Get
            Return _CardAmt
        End Get
        Set(value As String)
    ...
    Dim InCosCash, InCosCV, InCosGV, InCosCheck, InConNEFT, InConRTGS As Decimal
    ...
    Dim _float, _GVAmount, _CVAmount, _Check, _CreditCard, _TotalTillCollection, _VendorAmount, _NEFT, _RTGS As Decimal
    ...
    Public WriteOnly Property TotalFloat() As Decimal
        ...
        End Set
    End Property
    Public WriteOnly Property NEFT() As Decimal   'vipin on 24/04/2017
        Set(ByVal value As Decimal)
            _NEFT = value
        End Set
    End Property
    Public WriteOnly Property RTGS() As Decimal   'vipin on 24/04/2017
        Set(ByVal value As Decimal)
            _RTGS = value
        End Set
    End Property
    Private Sub frmShiftClosing_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If DTMain Is Nothing Then
                DTMain = ObjclsCommon.GetDenominationPC(clsAdmin.CurrencyCode, clsAdmin.CurrencyCode)
            End If
    ...
    Private Sub cbCurrency_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbCurrency.SelectedValueChanged
        Try
            ...
            If dv.Count <= 0 Then
                ObjclsCommon = New clsCommon()
                dt = ObjclsCommon.GetDenominationPC(cbCurrency.SelectedValue, clsAdmin.CurrencyCode)
    ...
    Private Sub cmdPrint_Click(sender As System.Object, e As System.EventArgs) Handles cmdPrint.Click
        ...
        Dim DtIncoData As New DataSet
        Dim DtTender As New DataTable

        DtTender.Columns.Add("OtherTenders", GetType(String))     'vipin 25-04-2017
        DtTender.Columns.Add("Amt", GetType(Integer))


        If _CardAmt <> "" Then
            DtTender.Rows.Add("Card", IIf((_CardAmt <> ""), _CardAmt, 0))
        End If


        If txtGVAmount.Text.ToString().Trim() <> "" Then
            '   If (_GVAmount - txtGVAmount.Value) >= 0 Then
            DtTender.Rows.Add("Gift Voucher", IIf((txtGVAmount.Text.ToString().Trim() <> ""), txtGVAmount.Text.ToString().Trim(), 0))
            'Else
            'DtTender.Rows.Add("Gift Voucher", IIf((txtGVAmount.Text.ToString().Trim() <> ""), _GVAmount - txtGVAmount.Value, 0))
            ' End If

        End If

        If txtCVAmount.Text.ToString().Trim() <> "" Then
            ' If (_CVAmount - txtCVAmount.Value) >= 0 Then
            DtTender.Rows.Add("Credit Voucher", IIf((txtCVAmount.Text.ToString().Trim() <> ""), txtCVAmount.Text.ToString().Trim(), 0))
            'Else
            '  DtTender.Rows.Add("Credit Voucher", IIf((txtCVAmount.Text.ToString().Trim() <> ""), _CVAmount - txtCVAmount.Value, 0))
            'End If

        End If

        If txtCheckAmount.Text.ToString().Trim() <> "" Then
            'If (_Check - txtCheckAmount.Value) >= 0 Then
            DtTender.Rows.Add("Cheque", IIf((txtCheckAmount.Text.ToString().Trim() <> ""), txtCheckAmount.Text.ToString().Trim(), 0))
            '  Else
            '   DtTender.Rows.Add("Cheque", IIf((txtCheckAmount.Text.ToString().Trim() <> ""), _Check - txtCheckAmount.Value, 0))
            ' End If
        End If

        If TxtNEFT.Text.ToString().Trim() <> "" Then
            'If (_NEFT - TxtNEFT.Value) >= 0 Then
            DtTender.Rows.Add("NEFT", IIf((TxtNEFT.Text.ToString().Trim() <> ""), TxtNEFT.Text.ToString().Trim(), 0))
            'Else
            '  DtTender.Rows.Add("NEFT", IIf((TxtNEFT.Text.ToString().Trim() <> ""), _NEFT - TxtNEFT.Value, 0))
        End If


        If TxtRTGS.Text.ToString().Trim() <> "" Then
            ' If (_RTGS - TxtRTGS.Value) >= 0 Then
            DtTender.Rows.Add("RTGS", IIf((TxtRTGS.Text.ToString().Trim() <> ""), TxtRTGS.Text.ToString().Trim(), 0))
            'Else
            ' DtTender.Rows.Add("RTGS", IIf((TxtRTGS.Text.ToString().Trim() <> ""), _RTGS - TxtRTGS.Value, 0))
            ' End If
        End If
        DtTender.TableName = "Tender"
        DtIncoData.Tables.Add(DtTender)
        '-------------------------------------------------------------------------------------------------------------------------------------

        Dim DtShiftName As New DataTable

        DtShiftName.Columns.Add("TerminalId", GetType(String))
        DtShiftName.Columns.Add("ShiftName", GetType(String))

        DtShiftName.Rows.Add(clsAdmin.TerminalID, shiftname.Substring(0, shiftname.IndexOf(" ")))
        DtShiftName.TableName = "ShiftDetail"
        DtIncoData.Tables.Add(DtShiftName)
        '-------------------------------------------------------------------------------------------------------------------------------------
        Dim dataTable = dgMainGrid.DataSource
        Dim dtDenomination As New DataTable

        If (TypeOf dataTable Is DataTable) Then
            dtDenomination = DirectCast(dataTable, DataTable)
        ElseIf (TypeOf dataTable Is DataView) Then
            dtDenomination = DirectCast(dataTable, DataView).ToTable()
        End If
        Dim dtNewDt As DataTable = dtDenomination.Copy

        If (dtNewDt.Columns.Count <> 3) Then
            dtNewDt.Columns.RemoveAt(1)
            dtNewDt.Columns.RemoveAt(1)
            dtNewDt.Columns.RemoveAt(2)
            dtNewDt.Columns.RemoveAt(2)
            dtNewDt.Columns.RemoveAt(3)
        End If

        'If (dtDenomination.Columns.Count <> 3) Then
        '    dtDenomination.Columns.RemoveAt(1)
        '    dtDenomination.Columns.RemoveAt(1)
        '    dtDenomination.Columns.RemoveAt(2)
        '    dtDenomination.Columns.RemoveAt(2)
        '    dtDenomination.Columns.RemoveAt(3)
        'End If

        dtNewDt.TableName = "Denomination"
        DtIncoData.Tables.Add(dtNewDt)

        '-------------------------------------------------------------------------------------------------------------------------------------

        GenerateShiftCloseInconsistenciesReport(shiftname, sitename, DtIncoData)   ' added by vipi on 26--4-2017
        '  Else
        ...
        Try
            ...
            If sender.tag = "Next" Then
                ...
                If ConsistancyDone = False Then
                    ...
                    If strIncos <> String.Empty Then  'vipin
                            ...
                            If strIncos.Trim() <> "" Then
                                If MsgBox(getValueByKey("TLC005") & strIncos, MsgBoxStyle.OkCancel, getValueByKey("CLAE04")) = MsgBoxResult.Ok Then
                                    ...
                                    If (clsDefaultConfiguration.ClientForMail = "PC") Then
                                        IncosAmt = IncosAmt + InCosCheck + InCosCV + InCosGV + InConNEFT + InConRTGS
                                    Else
    ...
    Private Sub cmdReset_Click(sender As System.Object, e As System.EventArgs) Handles cmdReset.Click
        ClearGrid()
        If Me.Text = getValueByKey("frmnshiftclosing") Then
    ...
    Private Function GenerateShiftCloseReport(ByVal shiftid As String) As Boolean
        Try
            ...
            reportViewer2.LocalReport.SetParameters(New ReportParameter() {SiteCodeParam, dateNameParam, TerminalParm, ShiftParm, ShiftOpenDateParm, ShiftCloseDateParm})
            ...
            If PrintFormat = "0" Then
                ...
                Dim DataSource11 As New ReportDataSource("WriteOffData", dsDsr.Tables(clsObj.ShiftCloseEnum.WriteOffData)) '' added by nikhil
                Dim DataSource12 As New ReportDataSource("NEFT", dsDsr.Tables(clsObj.ShiftCloseEnum.NEFT))   'vipin
                Dim DataSource13 As New ReportDataSource("RTGS", dsDsr.Tables(clsObj.ShiftCloseEnum.RTGS))
                ...
                reportViewer2.LocalReport.DataSources.Add(DataSource11) '' added by nikhil
                reportViewer2.LocalReport.DataSources.Add(DataSource12)  'vipin
            ...
            Else
                ...
                Dim DataSource11 As New ReportDataSource("WriteOffData", dsDsr.Tables(clsObj.ShiftCloseEnum.WriteOffData))  ''nikhil
                Dim DataSource12 As New ReportDataSource("NEFT", dsDsr.Tables(clsObj.ShiftCloseEnum.NEFT))   'vipin
                Dim DataSource13 As New ReportDataSource("RTGS", dsDsr.Tables(clsObj.ShiftCloseEnum.RTGS))
                ...
                reportViewer2.LocalReport.DataSources.Add(DataSource11) '' nikhil
                reportViewer2.LocalReport.DataSources.Add(DataSource12) 'vipin
                reportViewer2.LocalReport.DataSources.Add(DataSource13)
        ...
        End Try
    End Function
    Private Function GenerateShiftCloseInconsistenciesReport(ByVal shiftid As String, ByVal SiteName As String, ByVal DsDsr As DataSet) As Boolean
        Try
            ObjclsCommon = New clsCommon
            Dim shiftName As String
            shiftName = ObjclsCommon.GetShiftNameForPrint(shiftid, clsAdmin.SiteCode)
            ...
            ' Dim dsDsr As New DataSet()
            Dim clsObj As New ShiftCloseReport
            Dim DtDateTime As DataTable

            DtDateTime = clsObj.GetShiftCloseInConsistanciesReportData(clsAdmin.SiteCode, clsAdmin.DayOpenDate)
            DtDateTime.TableName = "DateTime"
            DsDsr.Tables.Add(DtDateTime)

            Dim path As String = ""
            Dim DtReportName As New DataTable   'vipin 15-05-2017
            DtReportName.Columns.Add("ReportName", GetType(String))
            DtReportName.TableName = "ReportName"
            If (Me.Text = "Shift Closing") Then

                DtReportName.Rows.Add("Shift Closing Denomination Details")
                path = clsDefaultConfiguration.DayCloseReportPath & "\" & SiteName & "_ShiftCloseInconsistencies_" & shiftName & "_" & DateTime.Now.ToString("dd-MM-yyyy_HH-mm-ss") & ".pdf"
            Else
                DtReportName.Rows.Add("Shift Close Float Details")
                path = clsDefaultConfiguration.DayCloseReportPath & "\" & SiteName & "_ShiftCloseFloatDetails_" & shiftName & "_" & DateTime.Now.ToString("dd-MM-yyyy_HH-mm-ss") & ".pdf"
            End If
            DsDsr.Tables.Add(DtReportName)

            Dim reportViewer1 As New LocalReport()
            Dim appPath As String = ""

            appPath = System.IO.Path.Combine(Application.StartupPath, "Reports\ShiftCloseInconsistenciesReport.rdl")
            '  appPath = System.IO.Path.Combine(Application.StartupPath, "Reports\Report2.rdl")

            reportViewer1.Refresh()
            reportViewer1.ReportPath = appPath
            ' reportViewer1.ProcessingMode = ProcessingMode.Local
            reportViewer1.DataSources.Clear()

            Dim DataSource1 As New ReportDataSource("Tender", DsDsr.Tables("Tender"))
            Dim DataSource2 As New ReportDataSource("ShiftDetail", DsDsr.Tables("ShiftDetail"))
            Dim DataSource3 As New ReportDataSource("Denomination", DsDsr.Tables("Denomination"))
            Dim DataSource4 As New ReportDataSource("DateTime", DsDsr.Tables("DateTime"))
            Dim DataSource5 As New ReportDataSource("ReportName", DsDsr.Tables("ReportName")) 'vipin 15-05-2017

            reportViewer1.Refresh()

            reportViewer1.DataSources.Add(DataSource1)
            reportViewer1.DataSources.Add(DataSource2)
            reportViewer1.DataSources.Add(DataSource3)
            reportViewer1.DataSources.Add(DataSource4)
            reportViewer1.DataSources.Add(DataSource5)

            reportViewer1.Refresh()


            Dim mybytes As [Byte]() = reportViewer1.Render("Pdf")
            'Byte[] mybytes = report.Render("PDF"); for exporting to PDF
            ' path = clsDefaultConfiguration.DayCloseReportPath & "\" & SiteName & "_ShiftCloseInconsistencies_" & shiftName & "_" & DateTime.Now.ToString("dd-MM-yyyy_HH-mm-ss") & ".pdf"
            Using fs As FileStream = File.Create(path)
                fs.Write(mybytes, 0, mybytes.Length)
    ...
    Private Sub Print()
        Try
            ...
            Else
                ...
                printDoc.PrinterSettings.PrinterName = SetPrinterName(dtPrinterInfo, "CashMemo", "Billing")
                printDoc.Print()
            End If
    ...
    '    End Try
    'End Sub
    ' Private Sub txtCVAmount_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtCVAmount.KeyDown, txtCheckAmount.KeyDown, txtGVAmount.KeyDown  'vipin
    Private Sub txtCVAmount_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtCVAmount.KeyDown, txtCheckAmount.KeyDown, txtGVAmount.KeyDown, TxtNEFT.KeyDown, TxtRTGS.KeyDown
    ...
    Private Function Check() As String
        ...
        Try
            ...
            If (clsDefaultConfiguration.ClientForMail = "PC") Then
                TxtNEFT.Text = IIf(TxtNEFT.Text = String.Empty, 0, TxtNEFT.Text)    'vipin on 24/04/2017
                TxtRTGS.Text = IIf(TxtRTGS.Text = String.Empty, 0, TxtRTGS.Text)
            End If
            ...
            For Each dr As DataRow In DtCurrency.Rows
                ...
                If SysAmt <> UserAmt Then
                    ...
                    str = str + "                            "    'vipin24/04/2017
                    '   str = str & dr("CURRENCYDESCRIPTION").ToString() & " :      " & InCosCash & vbCrLf
                    str = str & "Cash :           " & InCosCash & "                                         " & vbCrLf
            ...
            Dim DtCheckAmt As DataRow = dtTender.NewRow
            DtCheckAmt("TenderCode") = "Cheque"
            DtCheckAmt("Amount") = InCosCheck
            dtTender.Rows.Add(DtCheckAmt)
            '-----------------------------------------------------------------------------------------------vipin on 24/4/2017
            If (clsDefaultConfiguration.ClientForMail = "PC") Then

                If _NEFT <> TxtNEFT.Text Then
                    InConNEFT = _NEFT - TxtNEFT.Text
                    InConNEFT = FormatNumber(InConNEFT, 2)
                    str = str + "                           "    'vipin24/04/2017
                    str = str & "NEFT :           " & InConNEFT.ToString & "                                         " & vbCrLf
                End If

                Dim DtNeftAmt As DataRow = dtTender.NewRow
                DtNeftAmt("TenderCode") = "NEFT"
                DtNeftAmt("Amount") = InConNEFT
                dtTender.Rows.Add(DtNeftAmt)
                '-----------------------------------------------------------------------------------------------vipin on 24/4/2017


                If _RTGS <> TxtRTGS.Text Then
                    InConRTGS = _RTGS - TxtRTGS.Text
                    InConRTGS = FormatNumber(InConRTGS, 2)
                    str = str + "                           "    'vipin24/04/2017
                    str = str & "RTGS :           " & InConRTGS.ToString & "                                         " & vbCrLf

                End If

                Dim DtRtgsAmt As DataRow = dtTender.NewRow
                DtRtgsAmt("TenderCode") = "RTGS"
                DtRtgsAmt("Amount") = InConRTGS
                dtTender.Rows.Add(DtRtgsAmt)
                '----------------------------------------------------------------------------------------------- '
            End If
            ...
            If _GVAmount <> txtGVAmount.Text Then
                ...
                str = str + "               "    'vipin24/04/2017
                str = str & "GiftVoucher :           " & InCosGV.ToString & "                                         " & vbCrLf
