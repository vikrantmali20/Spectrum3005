...
Imports System.IO
Public Class SalesTaxDetailsReport
    ...
    Public Function GenerateDayCloseReport(request As SpectrumCommon.DayCloseReportModel, reportPath As String) As Boolean Implements IReports.GenerateDayCloseReport
        Try
            ...
            Dim ds As DataSet = GetTaxReportDataSet(request)
            If ds.Tables IsNot Nothing AndAlso ds.Tables.Count > 0 AndAlso Not String.IsNullOrEmpty(path) Then
                ...
                Dim sheet1 As ISheet = hssfworkbook.CreateSheet("Tax details report(sales)")
                Dim siteName As String = GetSiteName(request.SiteCode)
                Dim lastRow As IRow = sheet1.CreateRow(0)
                lastRow.CreateCell(0).SetCellValue("Name : Tax details report(sales)")
                ...
                lastRow.CreateCell(0).SetCellValue("Tax details report for period : " & request.FromDate.ToShortDateString() & " to " & request.ToDate.ToShortDateString())
                SetCellStyle(lastRow, GetBoldFont(hssfworkbook))
                ...
                For Each dr As DataRow In ds.Tables(2).Rows
                    lastRow.CreateCell(5).SetCellValue("Grand Total :")
                    lastRow.CreateCell(6).SetCellValue(dr(0).ToString())
                    lastRow.CreateCell(7).SetCellValue(dr(1).ToString())
                    lastRow.CreateCell(8).SetCellValue(dr(2).ToString())
                    lastRow.CreateCell(9).SetCellValue(dr(3).ToString())
                    lastRow.CreateCell(10).SetCellValue(dr(4).ToString())
                    SetCellStyle(lastRow, GetBoldFont(hssfworkbook))
                    row += 1
                    lastRow = sheet1.CreateRow(row)
    ...
    Private Function GetTaxReportDataSet(ByVal request As DayCloseReportModel) As DataSet
        Try
            Dim query As String = "exec USP_TaxDetailSales '" & request.FromDate.ToString("yyyy-MM-dd") & "' ,'" & request.ToDate.ToString("yyyy-MM-dd") & "' ,'''" & request.SiteCode & "'''"
    ...
    Private Function GetDirectoryPath(ByRef request As DayCloseReportModel) As String
        Try
            ...
            End If
            path = DayCloseReportPath & "\SalesTaxDetailsReport_" & siteName & "_" & request.FromDate.ToString("dd-MM-yyyy") & "_To_" & request.ToDate.ToString("dd-MM-yyyy") & ".xls"
            Return path
        Catch ex As Exception
            LogException(ex)
            Return Nothing
